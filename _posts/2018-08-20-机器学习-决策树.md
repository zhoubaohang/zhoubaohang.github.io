...
- 假定当前样本$D$中第$k$类样本所占的比例为$$p_k(k=1,2,\dots,\left | y \right |)$$,则$$D$$的信息熵定义为：
...
2. 假定属性$a$有$V$个可取值$\{a^1,a^2,\dots,a^V \}$，令$\tilde D^v$表示$D^v$中在属性$a$上取值为$a^v$的样本子集，则显然有$$\tilde D = \bigcup_{k=1}^{\left | y \right |} \tilde D_k , \tilde D = \bigcup_{v=1}^V \tilde D^v$$。
